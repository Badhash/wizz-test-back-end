...
const { isValidGameName, sanitizeGameName, isValidPlatform, arrayChunk} = require('./utils');
...
app.post('/api/games/populate', async (req, res) => {
  try {
    ...
    const BATCH_SIZE = 10;
    const allGamesChunked = arrayChunk(allGames, BATCH_SIZE);

    // Process each chunk sequentially tried with a Promise.all but it was too fast database locked
    for (const chunk of allGamesChunked) {
      for (const {
        publisherId,
        name,
        platform,
    ...
    res.status(200).send({ message: 'Data populated successfully' });
  } catch (error) {
